...
import ExtraFieldComponentsPage from './extra-field.page-object';
import { ExtraFieldDeleteDialog } from './extra-field.page-object';
...
describe('ExtraField e2e test', () => {
  ...
  it('should load create ExtraField page', async () => {
    ...
    expect(await extraFieldUpdatePage.getPageTitle().getAttribute('id')).to.match(/grupoamigoBackendApp.extraField.home.createOrEditLabel/);
  ...
  it('should delete last ExtraField', async () => {
    ...
    expect(await extraFieldDeleteDialog.getDialogTitle().getAttribute('id')).to.match(/grupoamigoBackendApp.extraField.delete.question/);
